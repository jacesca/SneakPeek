...
class Solution:
    def findAndReplacePattern(self,
                              words: List[str], pattern: str) -> List[str]:
        pattern = pattern.upper()
        result = []
        for w in words:
            repls = dict(map(reversed, dict(zip(pattern, w)).items()))
            wp = reduce(lambda a, kv: a.replace(*kv), repls.items(), w)
            if wp == pattern:
                # print(repls, pattern, w)
                result.append(w)
        return result
...
print(Solution().findAndReplacePattern(["a", "b", "c"], "a"))
